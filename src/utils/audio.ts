// Web Audio API Synthesizer for VMC Kiosk Tactile Audio Feedback
...
export function playTouchClick() {
  try {
...
export function playProcessingWhir() {
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
    const osc = ctx.createOscillator();
    ...
    osc.frequency.linearRampToValueAtTime(580, ctx.currentTime + 0.4);
...
export function startEmergencySiren() {
  ...
  try {
    ...
    const triggerTone = () => {
      ...
      const gain = ctx.createGain();
      osc.type = 'square';
      osc.frequency.setValueAtTime(toggle ? 960 : 770, ctx.currentTime);
    ...
    triggerTone();
    sirenInterval = setInterval(triggerTone, 320);
  } catch {
    //
  }
}
